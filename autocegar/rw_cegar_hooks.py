...
Wiring implemented here (previously scaffolded but unused):
  ...
  * warm-up     — for `warmup_epochs`, training runs as **plain RW-1** (correction
                  trained every epoch) with the CEGAR **gate OFF** (and the
                  _writeback_scale hook off); the gate switches on afterwards. NOT
                  forecaster-only — correction is trained throughout like RW-1.
...
class CNN_RW_CEGAR_HookBase(CNN_RW):
    ...
    def fit(self, data, train_idx=None):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            # correction trained EVERY epoch (RW-1); warm-up = RW-1 with the gate OFF
            ...
            for i in range(0, X_indices.shape[0], self.batch_size):
                ...
                if warm:
                    ...
                    # expose this batch's target indices [B, pred_len] so a proposal can
                    # map a per-timestep signal onto per-window wrongness.
            ...
            if correction.grad is not None:
                ...
                if not warm:   # write-back hook only once the gate is active (warm-up = plain RW-1)
